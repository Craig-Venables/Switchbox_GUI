"""
Single-file analyzer for memristor device measurements.

This module provides comprehensive analysis capabilities for individual
sweep files containing I-V measurements. It supports multiple analysis
levels from basic metrics to advanced research diagnostics, including:

- I-V loop characterization (Ron, Roff, ON/OFF ratio, hysteresis area)
- Device classification (memristive, ohmic, linear, etc.)
- Conduction mechanism identification (SCLC, Ohmic, etc.)
- Advanced metrics (switching energy, power consumption, etc.)
- Research diagnostics (NDR detection, kink voltage, loop similarity)

The analyzer is designed to work with predesigned sweep configurations
and can be integrated into larger batch processing systems.
"""
