...
class ReviewPanel:
    ...
    def _build_ui(self):
        ...
        ttk.Button(nav_frame, text="◄ Previous", command=self._previous_device).pack(side=tk.LEFT, padx=5)
        ttk.Button(nav_frame, text="Next ►", command=self._next_device).pack(side=tk.LEFT, padx=5)
    ...
    def _display_device(self, pred):
        ...
        if label:
            info_lines.extend([
                ...
                f"  {'✓ Correct' if classification.get('device_type') == label else '✗ Incorrect'}"
            ])
