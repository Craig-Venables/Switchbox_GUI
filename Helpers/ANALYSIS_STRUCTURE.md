...
2. If no tracking data exists, runs **retroactive analysis** on raw `.txt` files:
   ...
   - Uses: `Helpers.IV_Analysis.quick_analyze()` to generate device tracking/research data
3. Runs **comprehensive analysis**:
   - Imports: `Helpers.Sample_Analysis.comprehensive_analyzer.ComprehensiveAnalyzer`
...
3. For each file:
   ...
   - Runs: `Helpers.IV_Analysis.quick_analyze()` with `analysis_level='full'`
   - Uses: `Helpers.plotting_core.UnifiedPlotter` to generate:
...
2. For each device folder:
   ...
   - Loads data and runs: `Helpers.IV_Analysis.quick_analyze()` with `analysis_level='full'`
   - Uses: `Helpers.plotting_core.UnifiedPlotter.plot_iv_dashboard()` to generate dashboard plots
...
#### 1. **`Helpers/Sample_Analysis/`** - Sample-Level Analysis
...
- **Uses**: `Helpers.IV_Analysis.single_file_metrics.analyze_single_file` for individual file analysis
...
#### 2. **`Helpers/IV_Analysis/`** - Single File Analysis
...
#### 3. **`Helpers/plotting_core/`** - Plotting Utilities
...
#### 1. **`Helpers/Switchbox_Data_Analysis_and_Graphing - Copy/`**
- **Status**: Appears to be old/backup code
- **Evidence**: 
  - Folder name includes " - Copy"
  - Contains `data_analyzer.py` which was "restored" into `section_analyzer.py` (see comment in `section_analyzer.py:4`)
  - Contains `Old_code/` subfolder
- **Recommendation**: **Review and likely remove** if functionality is fully migrated

**Files to check**:
- `data_analyzer.py` - Functionality moved to `section_analyzer.py`
- `Main.py` - Old GUI? Check if still used
- `Old_code/` - Definitely old code
...
comprehensive_analyzer.py
    ├─> section_analyzer.py
    │   └─> Helpers.IV_Analysis.single_file_metrics.analyze_single_file
    └─> sample_analyzer.py
...
gui/measurement_gui/main.py
    ├─> Helpers.IV_Analysis.quick_analyze (for retroactive analysis & plotting)
    └─> Helpers.Sample_Analysis.comprehensive_analyzer.ComprehensiveAnalyzer
