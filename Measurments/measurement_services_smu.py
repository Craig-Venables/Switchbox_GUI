...
class MeasurementService:
        ...
        *,
        ...
        if hasattr(keithley, 'do_retention_measurement'):
            return keithley.do_retention_measurement(
                ...
                repeat_delay_s=float(repeat_delay_s),
                number=int(number),
                icc=float(icc),
                psu=psu,
                optical=optical,
    ...
    def run_ispp(
        ...
        *,
        ...
        """Incremental Step Pulse Programming until |I| reaches target (returns amp, I, t)."""
        if pulse_width_ms is None:
            try:
                pulse_width_ms = float(self.smu_limits.get_limits(smu_type).get("min_pulse_width_ms", 1.0))
            except Exception:
    ...
    def run_pulse_width_sweep(
        ...
        *,
        ...
        """Sweep width at fixed amplitude; returns (width_ms, I_read, t)."""
        out_w: List[float] = []
        out_i: List[float] = []
        out_t: List[float] = []
        t0 = time.time()
    ...
    def run_threshold_search(
        ...
        *,
        ...
        """Binary-search minimal |V| giving |I| ≥ target (returns tried V/I/t)."""
        if pulse_width_ms is None:
            try:
                pulse_width_ms = float(self.smu_limits.get_limits(smu_type).get("min_pulse_width_ms", 1.0))
            except Exception:
    ...
    def run_dc_capture(
        ...
        *,
        ...
        """Hold DC bias, sample I(t); returns (V, I, t) with V constant at bias."""
        v_arr: List[float] = []
        i_arr: List[float] = []
        t_arr: List[float] = []
    ...
    def run_transient_decay(
        ...
        *,
        ...
        """Single pulse then read and sample I(t); returns (t, I, V_read)."""
    ...
    def run_ppf(
        ...
        *,
        ...
        """Paired-pulse facilitation; returns (Δt, I1, I2, PPF index)."""
    ...
    def run_stdp(
        ...
        *,
        ...
        """STDP: pre/post pulses separated by Δt; returns (Δt, I0, I_after, Δw)."""
        # Enforce min width
        try:
            min_ms = float(self.smu_limits.get_limits(smu_type).get("min_pulse_width_ms", 1.0))
        except Exception:
    ...
    def run_srdp(
        ...
        *,
        ...
        """SRDP: pulse trains at various frequencies; returns (freq_hz, I_ss)."""
        try:
            min_ms = float(self.smu_limits.get_limits(smu_type).get("min_pulse_width_ms", 1.0))
        except Exception:
            min_ms = 1.0
    ...
    def run_pot_dep(
        ...
        *,
        ...
        """Potentiation/Depression; returns (cycle_idx, I_immediate, I_post, ratio)."""
    ...
    def run_frequency_response(
        ...
        *,
        ...
        """Frequency response via pulse train; returns averages or raw stream based on flag."""
    ...
    def run_bias_dependent_decay(
        ...
        *,
        ...
        """Repeat transient decay for each Vread and concatenate results (t, I, Vread)."""
    ...
    def update_smu_limits(self, smu_type: str, **kwargs):
        """Update timing limits for a specific SMU model (e.g., min pulse width)."""
        self.smu_limits.update_limits(smu_type, **kwargs)

    def get_smu_limits(self, smu_type: str) -> dict:
        """Return timing limits dict for a specific SMU model."""
    ...
    #     trig_mode: str = "BUS",
    # ) -> Tuple[List[float], List[float]]:
    #     """Apply a generator pulse and measure fast decay current at a DC bias using the SMU_AND_PMU/PMU.
    ...
    #     - sample_dt_s: sampling interval for SMU_AND_PMU current reads
