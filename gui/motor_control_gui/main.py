...
import Equipment.Motor_Controll.config as hw_config
from gui.motor_control_gui import config as gui_config
...
class MotorControlWindow:
    ...
    COLORS = gui_config.COLORS  # For handlers that use self.COLORS
    ...
    def __init__(
        ...
        self.presets_file = gui_config.PRESETS_FILE
        self._load_presets()
        ...
        self.var_velocity = tk.StringVar(value=str(hw_config.MAX_VELOCITY))
        self.var_acceleration = tk.StringVar(value=str(hw_config.MAX_ACCELERATION))
        ...
        self.laser_configs = gui_config.LASER_CONFIGS
    ...
    def _build_ui(self) -> None:
        ...
        create_header(self)
        create_controls_panel(self)
        create_canvas_and_camera(self)
    ...
    def _apply_motor_settings(self) -> None:
        ...
        try:
            ...
            # Reset to defaults
            self.var_velocity.set(str(hw_config.MAX_VELOCITY))
            self.var_acceleration.set(str(hw_config.MAX_ACCELERATION))
    ...
    def _stop_camera_feed(self) -> None:
        ...
        # Update UI (buttons may be absent if camera not available)
        if hasattr(self, "camera_start_button") and self.camera_start_button:
            self.camera_start_button.config(state=tk.NORMAL)
        if hasattr(self, "camera_stop_button") and self.camera_stop_button:
