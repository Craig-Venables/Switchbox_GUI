...
from Measurements.background_workers import (
...
from Measurements.pulsed_measurement_runner import PulsedMeasurementRunner
from Measurements.sequential_runner import run_sequential_measurement
from Measurements.single_measurement_runner import SingleMeasurementRunner
from Measurements.special_measurement_runner import SpecialMeasurementRunner
from Measurements.telegram_coordinator import TelegramCoordinator
from gui.measurement_gui.analysis_handlers import (
    format_analysis_value as _format_analysis_value_fn,
    get_latest_analysis_for_device as _get_latest_analysis_for_device_fn,
...
from gui.measurement_gui.conditional_testing import (
    run_conditional_testing as _run_conditional_testing,
    run_quick_test as _run_quick_test,
    run_tiered_test as _run_tiered_test,
    execute_single_sweep_for_conditional_test as _execute_single_sweep_for_conditional_test,
    update_conditional_testing_button_state as _update_conditional_testing_button_state,
)
from gui.measurement_gui.messaging_handlers import (
    load_messaging_data as _load_messaging_data,
    update_messaging_info as _update_messaging_info,
)
from gui.measurement_gui.custom_sweeps import (
    add_sweep_combination as _add_sweep_combination,
    delete_sweep_combination as _delete_sweep_combination,
    edit_sweep_combination as _edit_sweep_combination,
    load_custom_sweeps as _load_custom_sweeps_impl,
    load_custom_sweep_combinations as _load_custom_sweep_combinations,
    load_custom_sweep_methods as _load_custom_sweep_methods,
...
class MeasurementGUI:
    ...
    def __init__(self, master: tk.Misc, sample_type: str, section: str,
        ...
        self.custom_sweeps = _load_custom_sweeps_impl(str(_PROJECT_ROOT / "Json_Files" / "Custom_Sweeps.json"))
    ...
    def _run_analysis_if_enabled(
        ...
        """
        Post-measurement analysis pipeline.

        Always (when data is valid):
          - classification-level quick_analyze
          - device_tracking history JSON
          - device classification_log.txt
          - IV dashboard PNG (no conduction/SCLC during live measure)

        When Extended checkbox is ON, also:
          - GUI stats / top-bar label
          - analysis report txt under sample_analysis/

        Research-level re-analysis is not run here; use full-sample analysis on demand.
        """
        ...
        extended = bool(hasattr(self, 'analysis_enabled') and self.analysis_enabled.get())
        skip_extended = (
            is_custom_sequence
            and sweep_number > 1
            and not device_memristive_flag
        )
        ...
        try:
            ...
            if 'device_name' not in metadata:
                metadata['device_name'] = (
                    f"{self.sample_name_var.get()}_{self.final_device_letter}{self.final_device_number}"
                )
            ...
            debug_print(f"[ANALYSIS] Queuing classification on {len(v_arr)} points (extended={extended})...")
            ...
            def run_analysis_thread():
                try:
                    ...
                    memristivity_score = classification.get('memristivity_score', 0) or 0

                    try:
                        self._append_classification_log(save_dir, file_name, analysis_data)
                    except Exception as log_exc:
                        debug_print(f"[ANALYSIS] Failed to update classification log: {log_exc}")

                    sg = getattr(self, "sample_gui", None)
                    if sg is not None and hasattr(sg, "classification_overlay"):
                        root = getattr(sg, "root", None)
                        if root is not None:
                            root.after(0, sg.classification_overlay.refresh)
                        else:
                            sg.classification_overlay.refresh()
                    ...
                    def update_gui():
                        ...
                        if not is_custom_sequence:
                            try:
                                device_name = (
                                    f"{self.final_device_letter}{self.final_device_number}"
                                    if hasattr(self, 'final_device_letter')
                                    else "device"
                                )
                        ...
                        if not extended or skip_extended:
                            return
                        ...
                        if hasattr(self, 'plot_panels'):
                            self.plot_panels.log_graph_activity(
                                f"Analysis complete: {file_name} ({analysis_level})"
                            )
                    ...
                    if hasattr(self, 'master') and self.master:
                        def show_error():
                            try:
                                if hasattr(self, 'plot_panels'):
                                    self.plot_panels.log_graph_activity(
                                        f"Analysis failed: {str(exc)[:50]}"
                                    )
                            except Exception:
                                pass
                        self.master.after(0, show_error)

            _threading.Thread(target=run_analysis_thread, daemon=True, name="AnalysisThread").start()
            ...
            debug_print(f"[ANALYSIS ERROR] Failed to queue analysis: {exc}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _plot_measurement_in_background(
        ...
        *,
        ...
        _plot_measurement_in_background(
            self, voltage, current, timestamps, save_dir, device_name, sweep_number,
            is_memristive=is_memristive, filename=filename,
            measurement_type=measurement_type, measurement_params=measurement_params,
    ...
    def browse_sample_folder_for_analysis(self) -> None:
        ...
        _browse_sample_folder_for_analysis(self)
    ...
    def _run_analysis_sync(
        ...
        return _run_analysis_sync_fn(self, voltage, current, timestamps, device_id, save_dir)
    ...
    def update_messaging_info(self, _event: Optional[Any] = None) -> None:
        ...
        _update_messaging_info(self, _event)
