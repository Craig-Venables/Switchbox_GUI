    ...
    def _convert_argument_from_json(self, arg):
        ...
        if isinstance(arg, dict):
            if "type" in arg and "value" in arg:
                ...
                if arg_type in ctypes_map:
                    return ctypes_map[arg_type](arg_value)

                # fallback for built-in types
                return getattr(builtins, arg_type)(arg_value)

            # if it's a dictionary without 'type' and 'value', convert recursively
            return {k: self._convert_argument_from_json(v) for k, v in arg.items()}

        # if it's a primitive (int/float/etc.) just return
        return arg
