...
double time_offset=0; // offset to be added to each element of timearr
 ...
 // setV *= -1.0;
  int measCH = 2;
int p=0;
long long int total_time_u = time_limit*60*1000000;
while(1){
...
/*
  ...
  */
  ...
  // We add time_offset to each element of array before printing.This ensues linear time results.
       /*
       for(int k=0; k<numresetpts; ++k){
    timeArr[k] += time_offset;     
  }
  time_offset= timeArr[numresetpts-1] + timeArr[numresetpts-1] - timeArr[numresetpts-2];
  */
...
long tv1_sec; long tv1_usec;

tv1_sec = (long)((time-EPOCH)/10000000L);
tv1_usec = (long)(system_time.wMilliseconds * 1000);

long long int elapsed_time = ((tv1_sec-tv_sec)*1000000)+tv1_usec-tv_usec;
    //end time calculation

  PostDataDouble("TimeRES", elapsed_time);

  //PostDataDoubleBuffer("Time", timeArr, (numresetpts));
    if(elapsed_time>= total_time_u)goto RETURN;

  //Sweep Completed Successfully
  stat = TEST_SUCCESS;
p++;
  nlog("%s: exiting with status:%d\n", mod, stat);
    nlog("%ld",total_time_u);
