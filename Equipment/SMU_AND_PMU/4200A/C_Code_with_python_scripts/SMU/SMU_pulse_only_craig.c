/* USRLIB MODULE INFORMATION
	...
	MODULE NAME: SMU_pulse_only_craig
...
int SMU_pulse_only_craig( int initialize, int logMessages, double widthTime, double Amplitude, double Irange, double Icomp, double biasV, double biasHold, int *pulse_success )
{
/* USRLIB MODULE CODE */
        
// ************** NOTE: DO NOT TRY TO COMPILE WITH KXCI OPEN, it always give linker errors    **********************

        char mod[] = "SMU_pulse_only_craig";
        int stat = -1, j;
        int SMU1;
        int error=0;
...
pulse_success=&stat; //so clarius console message can be turned off and a value is still returned

    if (logMessages)
    {
    //printf("\n");
    printf("\n Hello - Pulse Only (No Measurement) \n");
...
// use mpulse to do a pulse and measrure at the same time
//double vsat,isat;
//mpulse(SMU1,Amplitude,widthTime, &vsat, &isat);      // mpulse (instr id, pulse_amplitude, pulse_duration, double * v_meas, double * i_meas)
...
stat=pulsev(SMU1,Amplitude, widthTime);
...
RETURN:
        ...
        if (logMessages) 
        {
         if (error==1) printf("Slew rate problem \n");
         printf("success:%d (0=yes, 1=no) \n", stat);
         }
        return stat;
/* USRLIB MODULE END  */
} 		/* End SMU_pulse_only_craig.c */
