/* USRLIB MODULE INFORMATION
	...
	MODULE NAME: ACraig5_PMU_retention
...
int ACraig5_retention_pulseNK( char *InstrName, long ForceCh, double ForceVRange, double ForceIRange, long MeasureCh, double MeasureVRange, double MeasureIRange, int max_pts, double MeasureBias, double *Volts, int volts_size, double *Times, int times_size, double *VF, int vf_size, double *IF, int if_size, double *VM, int vm_size, double *IM, int im_size, double *T, int t_size, int *npts );
...
This ACraig5 variant converts the original reset pulse into an additional read pulse so that
...
// are returned via of the voltage(VF and VM) and current (IF and IM) output arrays for both channels.
      ...
      stat = ACraig5_retention_pulse_ilimitNK
    ( inst, 
