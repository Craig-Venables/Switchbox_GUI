/* USRLIB MODULE INFORMATION
	...
	MODULE NAME: ACraig5_retention_pulse_ilimitNK
...
volatile long g_acraig5_wait_enabled = 0;
volatile long g_acraig5_user_ready = 1;
...
Module: ACraig5_retention_pulse_ilimitNK
=========================
...
/* USRLIB MODULE CODE */
  ...
  double t;
  int i, j;
  char mod[] = "ACraig5_retention_pulse_ilimitNK";
  long SeqList[1] = {1};
...
// ****************************************************
  ...
  if(g_acraig5_wait_enabled)
  {
    if(debug) printf("%s: waiting for user readiness flag\n", mod);
    while(!g_acraig5_user_ready)
    {
      Sleep(50);
    }
    g_acraig5_user_ready = 0;
    if(debug) printf("%s: user readiness acknowledged\n", mod);
  }
...
__declspec( dllexport ) int ret_getRate(double ttime, int maxpts, int *apts, int *npts) 
{
  ...
  *npts = (int) (ttime * usedrate + 0.5);
  ...
  return usedrate;
/* USRLIB MODULE END  */
} 		/* End ACraig5_retention_pulse_ilimitNK.c */
