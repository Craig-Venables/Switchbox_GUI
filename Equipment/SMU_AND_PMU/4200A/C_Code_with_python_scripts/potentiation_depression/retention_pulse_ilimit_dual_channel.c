/* USRLIB MODULE INFORMATION
	...
	MODULE NAME: retention_pulse_ilimit_dual_channel
...
Module: retention_pulse_ilimit_dual_channel
=========================
...
int retention_pulse_ilimit_dual_channel( char *InstrName, long ForceCh, double ForceVRange, double ForceIRange, double iFLimit, double iMLimit, long MeasureCh, double MeasureVRange, double MeasureIRange, int max_pts, double MeasureBias, double *Volts, int volts_size, double *Times, int times_size, double *VF, int vf_size, double *IF, int if_size, double *VM, int vm_size, double *IM, int im_size, double *T, int t_size, int *npts )
{
/* USRLIB MODULE CODE */
  int stat = 1;
  int status;
  ...
  double t;
  int i, j;
  char mod[] = "retention_pulse_ilimit_dual_channel";
  long SeqList[1] = {1};
...
__declspec( dllexport ) int ret_getRateWithMinSeg(double ttime, int maxpts, double min_seg_time, int *apts, int *npts) 
{
  ...
  *npts = 0;
  ...
  // Final check: if we found a rate but it violates segment time constraint, that's an error
  if(rate_found && max_rate_from_seg > 0 && usedrate > max_rate_from_seg)
  {
    if(debug) printf("%s: ERROR: Selected rate %d Hz violates minimum segment time constraint (max allowed: %d Hz)\n",
      mod, usedrate, max_rate_from_seg);
    usedrate = -3;
    goto RD;
  }
  ...
  *npts = (int) (ttime * usedrate + 0.5);
  ...
  if(debug)
  {
    printf("%s: used rate: %d used pts: %d allocate pts: %d maximum points: %d\n", mod, usedrate, *npts, *apts, maxpts);
    if(min_seg_time > 0.0)
    {
      double sample_period = 1.0 / (double)usedrate;
      double actual_min_seg_time = 5.0 * sample_period;
      printf("%s: sample period: %.2e s, minimum segment time supported: %.2e s\n", 
        mod, sample_period, actual_min_seg_time);
    }
  }

  return usedrate;
/* USRLIB MODULE END  */
} 		/* End retention_pulse_ilimit_dual_channel.c */
