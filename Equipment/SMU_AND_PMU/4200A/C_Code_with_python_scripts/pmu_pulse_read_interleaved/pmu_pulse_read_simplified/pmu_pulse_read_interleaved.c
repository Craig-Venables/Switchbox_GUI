...
/* USRLIB MODULE CODE */
  ...
  for (cycleIdx = 0; cycleIdx < NumPulses; cycleIdx++)
  {
    ...
    if(debug) printf("%s: Cycle %d - PULSE FALL: End v[%d]=%.6fV\n",
      mod, cycleIdx+1, segIdx, 0.0);

    // DELAY: Delay at 0V after pulse, before read (read delay)
    // CRITICAL: In ret_Define_SegmentsILimit, segment i goes from v[i] to v[i+1] over t[i]
    // So for delay: v[segIdx] must be 0.0 (start), v[segIdx+1] must be 0.0 (end), t[segIdx] = PulseDelay
    // Pattern: times[segIdx] = delay, volts[segIdx] stays 0.0 (already set), then segIdx++, then set volts[segIdx] = 0.0
    times[segIdx] = PulseDelay;
    ttime += times[segIdx];
    if(debug) printf("%s: Cycle %d - DELAY AT 0V (read delay): Start v[%d]=%.6fV, time t[%d]=%.9fs\n",
      mod, cycleIdx+1, segIdx, 0.0, segIdx, PulseDelay);
    segIdx++;
    volts[segIdx] = 0.0;  // END voltage of delay segment (flat at 0V) - CRITICAL: must be set AFTER increment
    if(debug) printf("%s: Cycle %d - DELAY AT 0V: End v[%d]=%.6fV (flat delay at 0V before read)\n",
      mod, cycleIdx+1, segIdx, 0.0);
    ...
    // Segment goes from v[segIdx] (0V, from delay end) to v[segIdx+1] (measV) over t[segIdx]
    times[segIdx] = riseTime;
    ttime += times[segIdx];
    if(debug) printf("%s: Cycle %d - READ RISE: Start v[%d]=%.6fV, time t[%d]=%.9fs\n",
      mod, cycleIdx+1, segIdx, 0.0, segIdx, riseTime);
    segIdx++;
    ...
    // DELAY: Delay at 0V after read, before next cycle (delay between cycles)
    // CRITICAL: In ret_Define_SegmentsILimit, segment i goes from v[i] to v[i+1] over t[i]
    // So for delay: v[segIdx] must be 0.0 (start), v[segIdx+1] must be 0.0 (end), t[segIdx] = measDelay
    // Pattern: times[segIdx] = delay, volts[segIdx] stays 0.0 (already set), then segIdx++, then set volts[segIdx] = 0.0
    times[segIdx] = measDelay;
    ttime += times[segIdx];
    if(debug) printf("%s: Cycle %d - DELAY AT 0V (between cycles): Start v[%d]=%.6fV, time t[%d]=%.9fs\n",
      mod, cycleIdx+1, segIdx, 0.0, segIdx, measDelay);
    segIdx++;
    volts[segIdx] = 0.0;  // END voltage of delay segment (flat at 0V) - CRITICAL: must be set AFTER increment
    if(debug) printf("%s: Cycle %d - DELAY AT 0V: End v[%d]=%.6fV (flat delay at 0V before next cycle)\n",
      mod, cycleIdx+1, segIdx, 0.0);
  }
