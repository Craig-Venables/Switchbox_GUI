/* USRLIB MODULE INFORMATION
	...
	MODULE NAME: pmu_endurance_interleaved
...
Module: pmu_endurance_interleaved
==================

Description
-----------
Endurance pattern: Initial Read, then (SET pulse, Read, RESET pulse, Read) x NumPulses.

SET amplitude: PulseV. RESET amplitude: setStopV.

Total measurements = 1 + 2 * NumPulses
...
Output arrays must be sized to accommodate: 1 + 2 * NumPulses measurements
...
/* USRLIB MODULE CODE */
  ...
  // Use maximum of PulseV, setStopV (RESET), and resetV for voltage range
  forceVRange = fabs(PulseV);
  if (fabs(setStopV) > forceVRange) forceVRange = fabs(setStopV);
  if (fabs(resetV) > forceVRange) forceVRange = fabs(resetV);
  ...
  // Pattern: Initial Read -> (SET -> Read -> RESET -> Read) x NumPulses
  int calc_times_count = 2 + 4;
  calc_times_count += 18 * NumPulses;
  ...
  // Probe capacity: 1 initial + 2 reads per endurance cycle
  probeCapacity = 1 + 2 * NumPulses;
  ...
  // ****************
  // Endurance: (SET -> Read -> RESET -> Read) x NumPulses
  // ****************
  int cycleIdx;
  int half;
  double progV;
  for (cycleIdx = 0; cycleIdx < NumPulses; cycleIdx++)
  {
    ...
    {
      progV = (half == 0) ? PulseV : setStopV;

      if(debug) printf("\n%s: ===== CYCLE %d/%d %s (progV=%.4fV) segIdx=%d =====\n",
        mod, cycleIdx+1, NumPulses, (half == 0) ? "SET" : "RESET", progV, segIdx);
      ...
      if(recordedProbeCount >= probeCapacity)
      {
        if(debug) printf("%s: Probe capacity exceeded at cycle %d half %d\n", mod, cycleIdx+1, half);
        stat = -205;
        goto RETS;
      }
      measMinTime[recordedProbeCount] = ttime + ratio * measWidth;
  ...
  if(debug) printf("%s: Endurance cycles complete (segIdx=%d)\n", mod, segIdx);
