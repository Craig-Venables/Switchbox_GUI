/* USRLIB MODULE INFORMATION
	...
	MODULE NAME: PMU_retention_single_channel
...
int ACraig4_retention_pulseNK( char *InstrName, long ForceCh, double ForceVRange, double ForceIRange, long MeasureCh, double MeasureVRange, double MeasureIRange, int max_pts, double MeasureBias, double *Volts, int volts_size, double *Times, int times_size, double *VF, int vf_size, double *IF, int if_size, double *VM, int vm_size, double *IM, int im_size, double *T, int t_size, int *npts );
...
extern int retention_pulse_single_channel_ilimitNK(char* InstrName, long ForceCh, double ForceVRange, double ForceIRange, double iFLimit, double iMLimit, long MeasureCh, double MeasureVRange, double MeasureIRange, long AuxCh, double AuxVRange, double AuxILimit, double AuxVLow, double AuxVHigh, double AuxRiseTime, double AuxFallTime, double AuxPulseWidth, double AuxPreDelay, double AuxPostDelay, int max_pts, double MeasureBias, double* Volte, int volts_size, double* Times, int times_size, double* VF, int vf_size, double *IF, int if_size, double *VM, int vm_size, double* IM, int im_size, double* T, int t_size, int* npts);
...
Module: PMU_retention_single_channel
==================
...
This ACraig7 variant converts the original reset pulse into an additional read pulse so that
...
both PMU channels for any iteration of the sweep. An optional auxiliary channel
can be driven with a simple seg_arb pulse defined by the Aux* parameters; this
auxiliary waveform is source-only (no measurements are collected).
...
AuxChannel 
:: Optional second PMU channel to pulse (set to 0 to disable).

AuxVRange 
:: Voltage range for the auxiliary channel.

AuxILimit 
:: Current compliance for the auxiliary channel (0.0 disables limiting).

AuxVLow / AuxVHigh 
:: Low and high levels for the auxiliary pulse.

AuxRiseTime / AuxFallTime 
:: Rise/fall times for the auxiliary pulse edges.

AuxPulseWidth 
:: Width of the auxiliary pulse high plateau.
...
int PMU_retention_single_channel( double riseTime, double resetV, double resetWidth, double resetDelay, double measV, double measWidth, double measDelay, double setWidth, double setFallTime, double setDelay, double setStartV, double setStopV, int steps, double IRange, int max_points, double *setR, int setR_size, double *resetR, int resetR_size, double *setV, int setV_size, double *setI, int setI_size, int iteration, double *out1, int out1_size, char *out1_name, double *out2, int out2_size, char *out2_name, double *PulseTimes, int PulseTimesSize, int NumbMeasPulses, long AuxChannel, double AuxVRange, double AuxILimit, double AuxVLow, double AuxVHigh, double AuxRiseTime, double AuxFallTime, double AuxPulseWidth, double AuxPreDelay, double AuxPostDelay, int ClariusDebug )
{
/* USRLIB MODULE CODE */
  char mod[] = "PMU_retention_single_channel";
  char inst[] = "PMU1";
  int i, stat;
  double *times = NULL;
  double *volts = NULL;
...
/* ------------------------------------------------------------------------------------------  */
// This calls retention_pulse_single_channel_ilimitNK in "retention_pulse_single_channel_ilimitNK.c".
// The force channel (PMU1) both sources and measures, while an optional auxiliary channel can source
// a simple seg_arb pulse (for the laser) without measurement. The segments are created based on the
// 'volts' and 'times' arrays and the force-channel voltage/current samples are returned in VF/IF.

      stat = retention_pulse_single_channel_ilimitNK
    ( inst, 
      (long) 1, forceVRange, IRange, 
      0.0, 0.0,
      (long) 0, measVRange, IRange,
      auxChannel, auxVRange, auxILimit,
      auxVLow, auxVHigh, auxRiseTime, auxFallTime, auxPulseWidth, auxPreDelay, auxPostDelay,
      max_points, 0.0, 
     ...
     {    
      ...
      stat = ret_find_value(IFret, Tret, numpts, setMinTime, setMaxTime, &resss);
      ...
      stat = ret_find_value(IFret, Tret, numpts, setMinTimeOff, setMaxTimeOff, &resss);
      ...
      stat = ret_find_value(IFret, Tret, numpts, resetMinTimeOff, resetMaxTimeOff, &resss);
...
for (ProbeResNumb=0; ProbeResNumb<recordedProbeCount;ProbeResNumb++)  
    {
      stat = ret_find_value(IFret, Tret, numpts, measMinTime[ProbeResNumb], measMaxTime[ProbeResNumb], &resss);
    ...
    //  *****   
      ...
      // get current
      // use IFret (force channel current) because single-channel mode measures on the forcing path
      stat = ret_find_value(IFret, Tret, numpts, measMinTime[ProbeResNumb], measMaxTime[ProbeResNumb], &resss);
      if(stat < 0)
        {
...
/* USRLIB MODULE END  */
} 		/* End PMU_retention_single_channel.c */
