...
void read_train_AllocateMemoryILimit(int npts);
void read_train_AllocateMeasArraysILimit(int npts);
void read_train_FreeMemoryILimit(void);
double read_train_Define_SegmentsILimit(double *v, double *t, int pts, double bias);
...
int read_train_ilimit( char *InstrName, long ForceCh, double ForceVRange, double ForceIRange, double iFLimit, double iMLimit, long MeasureCh, double MeasureVRange, double MeasureIRange, int max_pts, double MeasureBias, double *Volts, int volts_size, double *Times, int times_size, double *VF, int vf_size, double *IF, int if_size, double *VM, int vm_size, double *IM, int im_size, double *T, int t_size, int *npts )
{
/* USRLIB MODULE CODE */
  int stat = 1;
  int status;
  ...
  numpts = volts_size - 1;
  read_train_AllocateMemoryILimit(volts_size);
 ...
 if(debug) printf("Note: In the following segments, TTime is the time at the end of the segment so a probe measurement needs to be before this time\n");
   //Define Seqments:
  ttime = read_train_Define_SegmentsILimit(Volts, Times, volts_size - 1, MeasureBias);
  ...
  used_rate = read_train_getRate(ttime, max_pts, &allocate_pts, &NumDataPts);
  if(0 > used_rate)
  {
  ...
  read_train_AllocateMeasArraysILimit(allocate_pts);
...
double read_train_Define_SegmentsILimit(double *v, double *t, int pts, double bias)
{
  ...
  char mod[] = "read_train_Define_Segments";
  int i;
  double ttime;
