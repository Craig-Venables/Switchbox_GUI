...
try:
    # Try importing from file with space in name using importlib
    import importlib.util
    import os
    ammeter_file = os.path.join(os.path.dirname(__file__), 'Ammeter', 'Aligent A.py')
    ammeter_file = os.path.abspath(ammeter_file)
    if os.path.exists(ammeter_file):
        spec = importlib.util.spec_from_file_location("Aligent_A", ammeter_file)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        Agilent34401A = module.Agilent34401A
    else:
        raise ImportError(f"Could not find Agilent file at {ammeter_file}")
except Exception:
    # Fallback: try as if underscore (won't work but provides better error)
    try:
